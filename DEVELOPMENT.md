...
- **Rust:** BGitnuro is mainly written in Kotlin (JVM) but also uses Rust for some specific tasks. To set up your Rust
  environment,
  please read [its documentation](https://www.rust-lang.org/). `cargo` and `rustc` must be available in the path in
  order to build BGitnuro properly.
- **cargo-kotars:** A tool to run autogenerated bindings from Rust to Kotlin. You can install it using
  ...
  NOTE: Make sure cargo-kotars is in your path, otherwise BGitnuro's build will fail.
...
- `build.bat` (or `build.bat portable`) builds the portable app image into `dist\BGitnuro\BGitnuro.exe`.
- `build.bat installer` additionally tries to build an installer (requires Inno Setup).

The first run may take longer because it can automatically download/install missing tooling (JDK 17, cargo-kotars, Strawberry Perl, NASM).
